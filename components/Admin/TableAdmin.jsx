...
const TableAdmin = ({ novels }) => {
  ...
  const handleDeleteNovels = async () => {
    try {
      for (const novelId of selectedNovels) {
        const response = await fetch(`/api/admin/delete/${novelId}`, {
          method: "DELETE",
        });
        ...
        <Box className="w-full flex flex-row gap-4 bg-slate-200 p-4">
          <Box className="w-1/5 h-full flex flex-col bg-zinc-100 p-4 gap-4 rounded-md shadow-md ">
            ...
            <Button className="bg-main-bg-color rounded-md shadow-md hover:bg-rose-400 py-2 pl-4 gap-2">
              ...
              <Typography className="text-black" onClick={handleDeleteNovels}>
                Delete Novel
              </Typography>
            </Button>
          </Box>
              ...
              <Table>
                <TableHead className="bg-main-bg-color ">
                  <TableRow>
                    ...
                    <TableCell className="text-lg font-semibold">_id</TableCell>
                    <TableCell className="text-lg font-semibold">
                      Name
                    </TableCell>
                    <TableCell className="text-lg font-semibold">
                  ...
                  {novels &&
                    novels.map((novel) => (
                      <TableRow key={novel._id}>
                        <TableCell>
                          <Checkbox
                            checked={selectedNovels.includes(novel._id)}
                            onChange={() => handleCheckboxChange(novel._id)}
                          />
                        </TableCell>
                        <TableCell>
                          <Link
