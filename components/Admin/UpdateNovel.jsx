...
const UpdateNovel = ({ novel }) => {
  ...
  let panel = "";
  try {
    panel = novel.description[0].panel;
  } catch (error) {
    panel = "empty";
  }
  let href = "";
  ...
  let description = "";
  try {
    description = novel.description[1].description;
  } catch (error) {
    description = "empty";
  }
  let imgSrc = "";
  ...
  let synopsis = "";
  try {
    synopsis = novel.description[2].synopsis;
  } catch (error) {
    synopsis = "empty";
  }
  let author = "";
  ...
  const onSubmit = async (data) => {
    try {
      const response = await axios.post(`/api/admin/update/${novel._id}`, data);
      if (response.status === 200) {
